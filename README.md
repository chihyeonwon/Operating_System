...
현재 실행 중인 프로세스에게 허용된 최대 실행시간이 초과되었을때 CPU는 다른 프로세스를 선택하여 cpu를 보낼 수 있다.
-> 선점 cpu 스케줄링
...
용어 : 계산 위주(CPU Bounds) 프로세스, 입 출력 위주(I/O Bounds) 프로세스
비선점형 CPU 스케줄링 : 입추력이나 종료 등에 의한 자율적 CPU 반납만 실행
선점형 CPU 스케줄링 : 더 높은 우선순위나 프로세스나 시간 초과 등에 의한 타율적(강제적) CPU 반납도 시행
```
